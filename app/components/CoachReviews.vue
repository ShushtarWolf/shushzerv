...
const canReview = computed(() =>
  loggedIn.value && user.value?.role === 'ATHLETE',
)
...
<template>
  <section class="mt-8">
    ...
    <form v-if="canReview" class="mt-6 space-y-3 rounded-xl border border-brand-gray-200 p-4" @submit.prevent="submitReview">
      <h3 class="text-sm font-bold text-brand-gray-900">{{ t('reviews.writeCoach') }}</h3>
    ...
    <p v-else-if="!loggedIn" class="mt-4 text-sm text-brand-gray-500">

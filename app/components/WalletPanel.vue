...
<template>
  <section>
    <DashboardHeroCard
      ...
      :hint="t('clubs.currency')"
      :badge="t('dashboard.walletTab')"
    />

    <div v-else class="ios-card mb-6 overflow-hidden">
      <div class="bg-gradient-to-br from-brand-primary to-brand-orange p-6 text-white">
    ...
    <div
      v-if="canTopUp"
      class="mb-6 grid gap-3 p-4 sm:grid-cols-[1fr_auto]"
      :class="isFd ? 'fd-panel' : 'glass-panel'"
    >
    ...
    <h3 class="mb-3 font-bold" :class="isFd ? 'fd-section-title' : 'ios-title-3'">{{ t('wallet.transactions') }}</h3>
    <div v-if="transactions.length" class="space-y-2">
      <div
        v-for="tx in transactions"
        :key="tx.id"
        :class="isFd ? 'fd-transaction-row' : 'ios-card flex items-center justify-between gap-3 p-4'"
      >
        <div class="min-w-0">
          <p class="font-semibold text-fd-navy">{{ localized(tx.noteFa, tx.noteEn) || txLabel(tx.type) }}</p>
          <p class="text-sm" :class="isFd ? 'text-fd-muted' : 'ios-footnote'">{{ new Date(tx.createdAt).toLocaleDateString() }}</p>
        </div>
        <p
    ...
    <p v-else class="text-sm" :class="isFd ? 'text-fd-muted' : 'ios-footnote'">{{ t('wallet.noTransactions') }}</p>
  </section>
</template>

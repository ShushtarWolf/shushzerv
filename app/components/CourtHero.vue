...
const bgOpacity = computed(() => lerp(0.5, 0.12, easeInOutCubic(progress.value)))
const bgScale = computed(() => lerp(1.05, 1.18, progress.value))
...
const vignetteOpacity = computed(() => lerp(0.4, 0.72, progress.value))
</script>
...
<template>
  ...
  >
    <div
      class="relative w-full overflow-hidden bg-[#080605]"
    ...
    >
      <div
        ...
        :style="{
          backgroundImage: 'url(/images/hero-clay-tennis.jpg)',
          opacity: bgOpacity,
      ...
      <div
        class="pointer-events-none absolute inset-0 bg-gradient-to-b from-black/75 via-[#120a06]/40 to-black/92"
        :style="{ opacity: vignetteOpacity }"
      />
      <div class="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_at_50%_40%,transparent_0%,#080605_70%)]" />

      <ClientOnly>
        <ClayTennisCourtScene v-if="!prefersReducedMotion" :progress="progress" />
        <template #fallback>
          <div class="absolute inset-0 flex items-center justify-center">
            <div class="h-10 w-10 animate-pulse rounded-full border-2 border-white/15 border-t-orange-300/80" />
          </div>
        </template>
      </ClientOnly>
      ...
      >
        <p class="mb-3 text-[11px] font-semibold uppercase tracking-[0.32em] text-orange-300/90 sm:text-xs">
          {{ t('courtHero.eyebrow') }}
        </p>
      ...
      >
        ...
        <div class="h-9 w-5 rounded-full border border-white/20 p-1">
          <div class="mx-auto h-2 w-1 animate-bounce rounded-full bg-orange-200/70" />
        </div>
      </div>
    </div>
  </section>
